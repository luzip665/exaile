    ...
    def on_install_plugin_button_clicked(self, button):
        ...
        if result == Gtk.ResponseType.OK:
            ...
            def install(filename : str, overwrite_existing : bool = False) -> str:
                try:
                    plugin_name = self.plugins.install_plugin(filename, overwrite_existing)
                except plugins.PluginExistsError as e:
                    # self.message.show_question(_('Plugin file installation exists!'), str(e))
                    response = dialogs.yesno(self.window, "\n".join([_('Plugin allready exists!'), _('Do you want to override it?')]))
